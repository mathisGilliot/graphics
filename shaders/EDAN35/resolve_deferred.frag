...
void main()
{
	vec3 diffuse  = texture(diffuse_texture,  fs_in.texcoord).rgb; // k_d
	vec3 specular = texture(specular_texture, fs_in.texcoord).rgb; // k_s

	vec3 light_d  = texture(light_d_texture,  fs_in.texcoord).rgb;
	vec3 light_s  = texture(light_s_texture,  fs_in.texcoord).rgb;
	const vec3 ambient = vec3(0.15); // k_a
