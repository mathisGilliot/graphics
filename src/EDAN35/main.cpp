...
edan35::Assignment2::Assignment2()
{
	...
	window = Window::Create("EDAN35: Project", config::resolution_x,
...
edan35::Assignment2::run()
{
	...
	while (!glfwWindowShouldClose(window->GetGLFW_Window())) {
		...
		// For this frame, we need to clear the previous frame buffers
		...
		for (size_t i = 0; i < static_cast<size_t>(lights_nb); ++i) {
			...
			// XXX: Is any clearing needed?
			// For each light, we start from a new light "point"
			// So we need to clear the depth buffer
			glClear(GL_DEPTH_BUFFER_BIT);
